  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div className="flex w-full max-w-sm flex-col gap-3">
        <Row label="Content score" value={CONTENT_SCORE} color="bg-sky-400" />
        <Row label="Collaborative score" value={COLLAB_SCORE} color="bg-violet-400" />
        <Row label="Blended" value={blended} color="bg-accent" bold />
      </div>
      <div className="flex w-full max-w-sm items-center gap-3 text-xs text-muted">
...
function Row({
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <span className={`w-32 shrink-0 ${bold ? "font-medium text-foreground" : "text-muted"}`}>{label}</span>
